...
import time, math, requests, pandas as pd, os
from openpyxl import Workbook
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
from openpyxl.utils import get_column_letter
from pathlib import Path

# Save to the same folder as this script (i.e. Downloads)
...
# ── Main fetch loop ───────────────────────────────────────────────────────────
# If a previous run's CSV exists, load it and skip sets already fetched
existing_csv = OUT_DIR / "sv_meg_product_ids.csv"
if existing_csv.exists():
    existing_df = pd.read_csv(existing_csv, dtype=str)
    already_done = set(existing_df["group_id"].astype(str).unique())
    all_rows = existing_df.to_dict("records")
    print(f"♻️  Loaded {len(existing_df)} existing rows from previous run.")
    print(f"   Skipping {len(already_done)} already-fetched sets.\n")
else:
    already_done = set()
    all_rows = []
...
for i, (gid, (abbr, sname, era)) in enumerate(TARGET_SETS.items(), 1):
    if str(gid) in already_done:
        print(f"[{i:02d}/{total}] {sname} ({abbr})... ✓ already fetched, skipping")
        continue
...
# ── Save CSV ──────────────────────────────────────────────────────────────────
csv_out = OUT_DIR / "sv_meg_product_ids.csv"
...
xl_out = OUT_DIR / "sv_meg_product_ids.xlsx"
wb.save(xl_out)
