...
MAPPING = {
    ...
    "swsh5": "BST", "swsh6": "CRE", "swsh7": "CHP", "swsh8": "FUS",
...
VARIANT_MAP = {
    "norm": "N", "rev": "RH", "holo": "H",
    "1sted": "1E", "1stedh": "1E-H",
}

df = pd.read_csv(r"C:\Downloads\store_data_20260522_110808.csv", usecols=["sku","quantity","price"])
...
df["variant"] = df["var_raw"].map(VARIANT_MAP)
df = df[df["db_code"].notna() & df["variant"].notna() & df["quantity"] > 0]

print(f"Rows with stock to load: {len(df):,}")

print("Building DB lookup...")
lookup = {}
...
print(f"DB lookup size: {len(lookup):,}")
...
print(f"Done! Cards now in stock: {stocked:,}")
