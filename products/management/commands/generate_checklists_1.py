...
def generate_pdf(set_id, set_name, set_series, cards, out_path):
    ...
    for i, card in enumerate(cards):
        ...
        col_data[ci].append([f"[] {card.get('number',''):>4}", card.get("name",""), RARITY_SYMBOL.get(card.get("rarity",""), "")])
    ...
    story += [tbl, Spacer(1, 6*mm),
        Paragraph("Symbol Legend: circle=Common  diamond=Uncommon  star=Rare  starH=RareHolo  IR=IllustrationRare  UR=UltraRare  SR=SecretRare  PR=Promo", s_leg),
        Spacer(1, 3*mm),
...
def generate_excel(set_id, set_name, set_series, cards, out_path):
    ...
    for ri, card in enumerate(cards, 5):
        ...
        vals = [card.get("number",""), card.get("name",""), card.get("supertype",""),
                ", ".join(card.get("types",[]) or []), rarity, RARITY_SYMBOL.get(rarity,""), ""]
    ...
    ws.conditional_formatting.add(f"A5:G{last_row}", FormulaRule(
        formula=['$G5<>""'],
    ...
    for i, line in enumerate(["","1. Type anything in Have it column when you own a card.",
        "2. Row turns green automatically.","3. Filter Have it for blanks to find missing cards.","",
        "Set tiers:","  Simple set = Commons and Holos","  Base set = All numbered cards incl. Reverse Holos",
        "  Master set = Base set + Illustration Rares","  Grand Master = Master set + Promos","",
        "Visit pokebulk.co.za to shop for missing cards!"], 2):
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--set", dest="set_id", type=str, help="Single set ID e.g. swsh1")
    ...
    def handle(self, *args, **options):
        ...
        out = output_dir()
        qs = (CardSet.objects.filter(set_id=sid_filter) if sid_filter
              else CardSet.objects.all().order_by("release_date"))
        if not qs.exists():
        ...
        for card_set in qs:
            ...
            try:
                ...
                self.stdout.write(f"  {len(cards)} cards")
