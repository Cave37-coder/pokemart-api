...
# Replace the entire PayFastInitView post method
old = '''        data = {
...
new = '''        # Build data in exact PayFast required order
        name_first = (request.user.first_name or request.user.username or '')[:100].strip()
        name_last  = (request.user.last_name or '')[:100].strip()
        email      = (request.user.email or '')[:254].strip()
...
if old in content:
    ...
    print("PayFast data block updated")
else:
    print("NOT FOUND")
