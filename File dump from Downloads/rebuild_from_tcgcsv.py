...
WHAT THIS DOES:
  1. Reads tcgcsv_all_products.json (already fetched locally)
  2. For every card in every group:
     - Finds the correct DB CardSet (creating it if missing)
     - Finds existing DB record by card_number + variant (or tcgcsv_product_id)
     - If found  → stamps tcgcsv_product_id, fixes name, fixes variant. DONE.
     - If not found → creates new record with TCGCSV data
  3. Never touches: price, image_url, attacks, name_japanese, hp, abilities
  4. Never deletes anything

USAGE:
  python manage.py rebuild_from_tcgcsv                        # full run
  python manage.py rebuild_from_tcgcsv --gid 2948            # single group
  python manage.py rebuild_from_tcgcsv --gid 2948 --dry-run  # test
"""
...
SUBTYPE_MAP = {
    ...
    "":                     "H",   # TG / GG / Prize Pack / single-variant sets
}
...
# ---------------------------------------------------------------------------
# MASTER GROUP CONFIG
# gid → (db_code, era_code, set_name, release_date)
# Every group from TCGCSV that has cards
# ---------------------------------------------------------------------------
ERA_META = {
    "B1": ("WotC Base Era",        1),
    "B2": ("EX Era",               2),
...
# Groups where empty subTypeName = Holo (single variant sets)
SINGLE_HOLO_GROUPS = frozenset({
    3020, 3068, 3172, 17674, 17689,   # Trainer Galleries + CRZ GG
    22880,                             # Prize Pack
    2931,                              # Celebrations Classic Collection
    3179, 23266, 23561,               # Trick or Trade
    2776,                              # First Partner Pack
})
...
def get_number(extended_data):
    for item in (extended_data or []):
        if item.get("name", "").lower() == "number":
            return str(item.get("value", "")).strip()
    return None

def get_rarity(extended_data):
    for item in (extended_data or []):
        if item.get("name", "").lower() == "rarity":
            return str(item.get("value", "")).strip()
    return ""

def build_display_name(tcg_name, variant):
    if variant == "RH":
        return f"{tcg_name} (Reverse Holo)"
    elif variant == "H":
        return f"{tcg_name} (Holo)"
    return tcg_name
...
class Command(BaseCommand):
    help = "Rebuild card DB from local TCGCSV data. Stamps tcgcsv_product_id on existing records, creates missing ones."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--data-file", default="tcgcsv_all_products.json",
            help="Path to tcgcsv_all_products.json (default: current dir)")
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(data_file):
            raise CommandError(
                ...
                f"Run fetch_tcgcsv.py first to download TCGCSV data."
            )
        ...
        self.stdout.write(f"  {len(tcgcsv_data)} groups loaded")

        # Get or create Cards category
        ...
        if gid_filter:
            ...
            work = {str(gid_filter): tcgcsv_data[str(gid_filter)]}
        else:
            work = tcgcsv_data

        # Filter to only groups in GROUP_CONFIG
        configured = {str(k): v for k, v in GROUP_CONFIG.items()}
        skipped_groups = []
        final_work = {}
        for gid_str, data in work.items():
            if gid_str in configured:
                final_work[gid_str] = data
            else:
                if data.get("cards"):
                    skipped_groups.append((gid_str, data.get("name","?"), len(data.get("cards",[]))))

        if skipped_groups and not gid_filter:
            self.stdout.write(self.style.WARNING(
                f"\nSkipping {len(skipped_groups)} unconfigured groups (not in GROUP_CONFIG):"
            ))
            for gid_str, name, count in skipped_groups:
                self.stdout.write(f"  gid={gid_str} {name} ({count} cards)")

        self.stdout.write(f"\nProcessing {len(final_work)} configured groups...\n")
        ...
        for i, (gid_str, data) in enumerate(
            sorted(final_work.items(), key=lambda x: int(x[0])), 1
        ):
            gid = int(gid_str)
            cfg = GROUP_CONFIG[gid]
            ...
            is_single_holo = gid in SINGLE_HOLO_GROUPS

            # Ensure set exists
            card_set, set_created = get_or_create_set(db_code, era_code, set_name, release_date)
            if set_created:
                total_sets_created += 1
                self.stdout.write(self.style.SUCCESS(f"  ✚ Created new set: {db_code} — {set_name}"))

            self.stdout.write(
                f"[{i}/{len(final_work)}] {db_code:12} {set_name:45} {len(cards):4} cards"
            )

            # Build lookup of existing DB cards for this set
            # Key: (card_number, variant) → PokemonProduct
            existing_by_num_var = {}
            existing_by_tcgid = {}
            for p in PokemonProduct.objects.filter(card_set=card_set):
                num = str(p.card_number or "").strip()
                var = str(p.variant_override or "N").strip()
                existing_by_num_var[(num, var)] = p
                if p.tcgcsv_product_id:
                    existing_by_tcgid[p.tcgcsv_product_id] = p
            ...
            for card in cards:
                ...
                card_number = card.get("_number", "")
                rarity_raw = card.get("_rarity", "")
                rarity = RARITY_MAP.get(rarity_raw, "common")

                subtype = (card.get("subTypeName") or "").strip()
                if is_single_holo and subtype == "":
                    variant = "H"
                else:
                    variant = SUBTYPE_MAP.get(subtype, "N")

                num_key = str(card_number).strip()

                # ── Find existing record ─────────────────────────────────
                existing = existing_by_tcgid.get(pid)

                if not existing:
                    existing = existing_by_num_var.get((num_key, variant))

                if not existing and variant == "N":
                    # Some old records stored as "" or missing variant
                    existing = existing_by_num_var.get((num_key, ""))

                if existing:
                    # Stamp the tcgcsv_product_id, fix name if it was wrong
                    changed = False
                    if existing.tcgcsv_product_id != pid:
                        existing.tcgcsv_product_id = pid
                        changed = True
                    # Fix variant if it was wrong (but NOT ball variants — preserve BRH-*)
                    current_var = existing.variant_override or ""
                    if not current_var.startswith("BRH") and current_var not in ("ERH", "TRH", "GS", "GX"):
                    ...
                    # Create new record
                    display_name = build_display_name(tcg_name, variant)
                    num_padded = str(card_number).zfill(3) if card_number else "000"
                    pb_id = f"PB-{db_code}-{variant}-{num_padded}"
                    ...
                    new = PokemonProduct(
                        ...
                        price=0,
                        stock=0,
                        is_active=False,  # inactive until priced
                    )
                ...
                if to_update:
                    with transaction.atomic():
                        PokemonProduct.objects.bulk_update(
                            to_update, ["tcgcsv_product_id", "variant_override"]
                        )
                if to_create:
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f"  Existing records stamped:  {total_stamped}\n"
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"All records now have tcgcsv_product_id set.\n"
                f"Run sync_prices next to set prices on all records."
            ))
